...
<body>
    ...
    @foreach ($dtrs as $i => $dtr)
        ...
        @php
            ...
            $specialHolidays = array_filter($dtr['entries'], fn($e) => $e['holidayType'] === 'specialWorkingHoliday');
            $dailyRateNum = (float) $dtr['dailyRateBasis'];
        @endphp
        ...
        @if (count($regularHolidays) > 0)
            <div class="overtime-note">
                ...
                @foreach ($regularHolidays as $entry)
                    @php $computed = (float) ($entry['baseRate'] ?? $dailyRateNum) * 2; @endphp
        ...
        @if (count($specialHolidays) > 0)
            <div class="overtime-note">
                ...
                @foreach ($specialHolidays as $entry)
                    @php $computed = (float) ($entry['baseRate'] ?? $dailyRateNum) * 1.3; @endphp
        ...
        @if ($dtr['totalOvertimeMinutes'] > 0)
            @php
                $hourlyRateBasis = $dailyRateNum > 0 ? $dailyRateNum / 8 : 0;
                $totalHours = $dtr['totalOvertimeMinutes'] / 60;
            ...
            <div class="overtime-note">
                ...
                {{ $dtr['totalOvertimeMinutes'] }} mins / 60 = {{ number_format($totalHours, 2) }} hours.
                    ...
                    <tr class="{{ $rowClass }}">
                        ...
                        <td>
                            @switch($entry['holidayType'])
                                @case('regularHoliday') Regular Holiday @break
                                @case('specialWorkingHoliday') Special Non Working Day @break
                                @default None
                            @endswitch
                        </td>
